...
var $esfuse$: EsfuseRuntime;

(() => {
  // @ts-expect-error
  if (typeof $esfuse$ !== `undefined`)
    return;
  ...
  $esfuse$ = {
    ...
    meta(newMeta) {
      Object.assign($esfuse$.currentMeta, newMeta);
    },
    ...
    require: Object.assign((p: string) => {
      ...
      if (moduleInfo.module === null) {
        ...
        const moduleMeta = $esfuse$.currentMeta[p];
        if (moduleMeta.error)
